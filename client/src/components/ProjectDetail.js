...
const ProjectDetail = ({ projectId }) => {
  ...
  const loadAnalytics = async () => {
    try {
      const response = await fetch(
        `/api/business-projects/${projectId}/analytics`,
      );
      const data = await response.json();
  ...
  const updateRevenue = async (amount, period = 'total') => {
    try {
      const response = await fetch(
        `/api/business-projects/${projectId}/revenue`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex space-x-2'>
          ...
          <Button onClick={() => setShowAddApi(true)}>Add API</Button>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        ...
        <Card className='p-4'>
          <div className='text-2xl font-bold text-blue-600'>
            {formatCurrency(project.revenue.monthly)}
          </div>
          <div className='text-sm text-gray-600'>Monthly Revenue</div>
        </Card>
        <Card className='p-4'>
          <div className='text-2xl font-bold text-purple-600'>
            {formatCurrency(project.revenue.target)}
          </div>
            ...
            <div>
              ...
              <div className='w-full bg-gray-200 rounded-full h-2'>
                <div
                  ...
                  style={{
                    width: `${Math.min(analytics.revenue.progress, 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            >
              <div>
                ...
                <div className='text-xs text-gray-500'>
                  {api.usage.calls} calls • {formatCurrency(api.usage.cost)}{' '}
                  cost
                </div>
              </div>
      ...
      {analytics && (
        <Card className='p-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            <div>
              <div className='text-2xl font-bold text-blue-600'>
                {analytics.apis.total}
              </div>
              <div className='text-sm text-gray-600'>Total APIs</div>
            </div>
            <div>
              <div className='text-2xl font-bold text-green-600'>
                {analytics.apis.calls}
              </div>
              <div className='text-sm text-gray-600'>API Calls</div>
            </div>
            <div>
              <div className='text-2xl font-bold text-purple-600'>
                {formatCurrency(analytics.apis.cost)}
              </div>
