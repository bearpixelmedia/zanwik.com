...
const SubscriptionManager = () => {
  ...
  const loadSubscriptions = async () => {
    try {
      const response = await fetch(
        '/api/monetization/users/current-user/subscriptions',
      );
  ...
  const loadUserFeatures = async () => {
    try {
      const response = await fetch(
        '/api/monetization/users/current-user/features',
      );
      const data = await response.json();
  ...
  const cancelSubscription = async subscriptionId => {
    ...
    try {
      const response = await fetch(
        `/api/monetization/subscriptions/${subscriptionId}/cancel`,
        {
          method: 'POST',
        },
      );
            ...
            <div className='mb-6'>
              ...
              <div className='flex items-center space-x-3'>
                ...
                <span className='text-sm text-gray-600'>
                  {formatCurrency(userFeatures.sponsorshipTier.price)}/
                  {userFeatures.sponsorshipTier.period}
                </span>
              </div>
            </div>
          ) : (
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <div className='text-2xl font-bold'>
                {userFeatures.limits.featuredApis === -1
                  ? 'Unlimited'
                  : userFeatures.limits.featuredApis}
              </div>
            </div>
            <div>
