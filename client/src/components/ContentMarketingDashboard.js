  ...
  const loadContentMarketingData = async () => {
    try {
      ...
      const scheduledResponse = await fetch(
        '/api/content-marketing/scheduled-posts',
      );
    ...
    >
      ...
      >
        {[
          {
            title: 'Blog Views',
            value: contentStats.blogViews,
            icon: '📊',
            color: '#667eea',
          },
          {
            title: 'Social Engagement',
            value: contentStats.socialEngagement,
            icon: '💬',
            color: '#764ba2',
          },
          {
            title: 'Conversions',
            value: contentStats.conversions,
            icon: '🎯',
            color: '#f093fb',
          },
          {
            title: 'Total Reach',
            value: contentStats.reach,
            icon: '👥',
            color: '#4facfe',
          },
        ].map((stat, index) => (
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <button
            onClick={() =>
              handlePostNow('linkedin', 'API Integration Guide 2024')
            }
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          <button
            onClick={() =>
              handlePostNow('reddit', 'API Security Best Practices')
            }
            style={{
              display: 'flex',
              alignItems: 'center',
