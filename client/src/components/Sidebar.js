...
const Sidebar = () => {
  ...
  useEffect(() => {
    const sampleRecent = [
      { name: 'E-commerce App', type: 'project', path: '/projects/1', icon: FolderOpen },
      { name: 'Analytics Dashboard', type: 'page', path: '/analytics', icon: BarChart3 },
      { name: 'User Management', type: 'page', path: '/users', icon: Users },
      { name: 'Server Monitoring', type: 'page', path: '/monitoring', icon: Monitor }
    ];
    setRecentItems(sampleRecent);
  }, []);
  ...
  const filteredNavigation = Object.entries(navigation).reduce((acc, [key, section]) => {
          ...
          <div className="flex items-center space-x-2">
            <div className="relative">
              ...
              {sessionInfo?.willExpireSoon && (
                <div className="absolute -top-1 -right-1 w-3 h-3 bg-yellow-500 rounded-full border-2 border-card animate-pulse"></div>
              )}
            </div>
            <div>
                      ...
                      {!isCollapsed && (
                        <button
                          ...
                          className={cn(
                            ...
                            isFavorite(item.href) ? "text-yellow-500" : "text-muted-foreground hover:text-yellow-500"
                          )}
                        >
