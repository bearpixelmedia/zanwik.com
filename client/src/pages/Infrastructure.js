...
const Infrastructure = () => {
  ...
  const handleDeployService = async (serviceData) => {
    try {
      setDeploying(true);
      await infrastructureAPI.deployService(serviceData);
      fetchInfrastructureData();
  ...
  const handleServiceAction = async (serviceId, action) => {
    try {
      switch (action) {
        case 'start':
          await infrastructureAPI.startService(serviceId);
          break;
        case 'stop':
          await infrastructureAPI.stopService(serviceId);
          break;
        case 'restart':
          await infrastructureAPI.restartService(serviceId);
          break;
    ...
    <div className="p-6 space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              >
                Healthy ({infrastructureData.services.filter(s => s.status === 'healthy').length})
              </Button>
              <Button
              ...
              >
                Warning ({infrastructureData.services.filter(s => s.status === 'warning').length})
              </Button>
              <Button
              ...
              >
                Error ({infrastructureData.services.filter(s => s.status === 'error').length})
              </Button>
            </div>
          </div>
        </CardHeader>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getDeploymentStatusColor(service.healthCheck)}`}>
                          {service.healthCheck}
                        </span>
                      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest service deployments and their status</CardDescription>
        </CardHeader>
        <CardContent>
          {infrastructureData.deployments.length > 0 ? (
            <div className="space-y-4">
              {infrastructureData.deployments.map((deployment) => {
                const service = infrastructureData.services.find(s => s.id === deployment.serviceId);
                return (
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="text-sm font-medium text-foreground">
                          {service?.name || 'Unknown Service'} - {deployment.version}
                        </h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md mx-4">
            <CardHeader>
              ...
              <CardDescription>Deploy a new service or update existing one</CardDescription>
            </CardHeader>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Service details and management</CardDescription>
                </div>
                <Button 
                  variant="outline" 
                  size="sm"
            ...
            <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className={getStatusColor(selectedServiceForAction.status)}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={getHealthCheckColor(selectedServiceForAction.healthCheck)}>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-3 border rounded">
                    <div className="text-2xl font-bold text-blue-500">{selectedServiceForAction.cpu}%</div>
                    <div className="text-sm text-muted-foreground">CPU</div>
                  </div>
                  <div className="text-center p-3 border rounded">
                    <div className="text-2xl font-bold text-green-500">{selectedServiceForAction.memory}%</div>
                    <div className="text-sm text-muted-foreground">Memory</div>
                  </div>
                  <div className="text-center p-3 border rounded">
                    <div className="text-2xl font-bold text-purple-500">{selectedServiceForAction.disk}%</div>
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <Button
                    variant="outline"
                    onClick={() => handleServiceAction(selectedServiceForAction.id, 'start')}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => handleServiceAction(selectedServiceForAction.id, 'stop')}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => handleServiceAction(selectedServiceForAction.id, 'restart')}
                  >
