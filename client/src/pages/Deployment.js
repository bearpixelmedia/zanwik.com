...
const Deployment = () => {
  ...
  const fetchDeploymentData = async () => {
    try {
      ...
      const mockData = {
        ...
        environments: [
          { name: 'production', status: 'healthy', deployments: 45, uptime: 99.9 },
          { name: 'staging', status: 'healthy', deployments: 23, uptime: 99.8 },
          { name: 'development', status: 'warning', deployments: 67, uptime: 98.5 }
        ],
        stats: {
  ...
  const getEnvironmentIcon = (environment) => {
    switch (environment) {
      ...
      case 'staging': return <Database className="h-4 w-4 text-yellow-500" />;
      case 'development': return <Globe className="h-4 w-4 text-blue-500" />;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your CI/CD pipelines and deployments.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {deploymentData.stats.activePipelines}/{deploymentData.stats.totalPipelines}
            </div>
            <div className="text-xs text-muted-foreground">
              Pipelines running
            </div>
        ...
        <CardContent>
          <div className="space-y-4">
            {filteredDeployments.map((deployment) => (
              <div key={deployment.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-accent transition-colors">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      <h3 className="text-sm font-medium text-foreground">{deployment.name}</h3>
                      <span className={`text-xs px-2 py-1 rounded-full ${getStatusBadge(deployment.status)}`}>
                        {deployment.status}
                      </span>
                    </div>
                ...
                <div className="flex items-center space-x-6">
                  <div className="text-right text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <span>Tests: {deployment.tests.passed}/{deployment.tests.total}</span>
            ...
            {deploymentData.recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-start space-x-3 p-3 border rounded-lg">
                <div className="flex-shrink-0 mt-1">
                  {getStatusIcon(activity.status === 'success' ? 'completed' : activity.status === 'running' ? 'in-progress' : 'failed')}
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md mx-4">
            <CardHeader>
              ...
              <CardDescription>Deploy a new version to an environment</CardDescription>
            </CardHeader>
